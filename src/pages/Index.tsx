...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col overflow-hidden safe-area-top safe-area-bottom">
      <header className="flex items-center justify-between px-8 safe-area-x py-2 border-b border-border bg-card shrink-0">
        <div className="flex items-center px-4 gap-3 min-w-0 flex-1">
          <button
            onClick={handleGoHome}
            className="p-3 px-4 rounded-md bg-secondary text-secondary-foreground shadow-xl shadow-gray-900/30 active:scale-95"
          >
        ...
        <div className="flex items-center px-8 gap-3 shrink-0">
          {isMaster && (
            <button
              onClick={handleShareBle}
              className={`flex items-center gap-2 px-4 py-2 shadow-xl shadow-gray-900/30 active:scale-95 rounded-md text-[22px] font-semibold transition-colors ${
                isSharingActive
          ...
          <Badge
            ...
            className="text-[20px] px-3 py-0.5 select-none"
          >
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 px-4 safe-area-x py-2 bg-muted/50 border-b border-border bg-card shrink-0 min-h-[56px]">
        {selectedIndex !== null && selectedInfo ? (
          <>
            <div className="flex items-center flex-wrap gap-x-4 gap-y-2 min-w-0 flex-1">
              <span className="px-8 text-[22px] font-bold text-foreground">#{selectedLabel}</span>
              <span className="text-[22px] text-field-fils font-semibold">
                Fils: <span className="font-medium">{selectedInfo.fils || "—"}</span>
              </span>
              <span className="text-[22px] text-field-borne font-semibold">
                Borne: <span className="font-medium">{selectedInfo.borne || "—"}</span>
              </span>
              <span className="text-[22px] text-field-bornier font-semibold">
                Bornier: <span className="font-medium">{selectedInfo.bornier || "—"}</span>
              </span>
              <span className="text-[22px] text-amber-700 font-semibold">
                Cf/Cm: <span className="font-medium">{selectedInfo.cfCm || "—"}</span>
              </span>
              {selectedInfo.locked && (
                <Badge
                  variant="outline"
                  className="text-[18px] px-2 py-0 border-state-locked text-muted-foreground"
                >
            ...
            <div className="flex items-center gap-3 ml-auto shrink-0">
              <Button
                ...
                variant="outline"
                size="sm"
                className="h-12 px-4 text-[22px] gap-2"
              >
          ...
          <>
            <span className="px-8 text-[22px] text-muted-foreground mr-2 min-w-0 flex-1">
              Appuyez sur un bouton pour voir ses infos
            </span>
            <div className="flex items-center px-8 gap-3 ml-auto shrink-0">
              <Button
                onClick={handleExportReport}
                variant="outline"
                size="sm"
                className="h-12 px-4 text-[22px] gap-2 shadow-xl shadow-gray-900/30 active:scale-95"
              >
                <FileChartColumn className="w-6 h-6" />
                Rapport
              </Button>
              <Button
                ...
                variant="outline"
                size="sm"
                className="h-12 px-4 text-[22px] gap-2 shadow-xl shadow-gray-900/20 active:scale-95"
              >
                <List className="w-6 h-6" />
                Légende
              </Button>
